<template>
    <header class="z-20 px-[20px] md:px-[50px] lg:px-[140px] h-[70px] flex items-center shadow-md justify-between sticky top-0 bg-[#FFFC] backdrop-blur-md">
        ...
        <div v-else class="relative hidden sm:block">
            ...
            |
            <button class="ms-4 p-3 py-2 bg-[#E5A00A] text-[#FFF] rounded-full px-5 hover:shadow-special hover:bg-white hover:bg-transparent hover:text-[#463691] transition-all duration-300" @click.stop="isCollapsed = !isCollapsed">Moje konto</button>
        ...
        <div class="block sm:hidden absolute left-0 top-[70px] bg-[#FFFE] backdrop-blur-md w-full shadow-lg pt-5 pb-5 px-4" v-show="isMobileCollapsed">
